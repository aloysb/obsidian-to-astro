    ...
    return notes.map((note) => {
        ...
        note.content = lines.map((line) => {
            ...
            function processLink(link: string): string {
                ...
                if (!linkedNote) {
                    console.log(`${note.filePath} has a link to '${file}' that we could not resolve. The link has been ignored.`)
                    const replace = `${title ?? file}`;
                    return replace
                }
                const replace = `[${title ?? file}](../${linkedNote.frontmatter.slug}.md)`;
                return replace
            }
        }).join('\n');
        return note;
